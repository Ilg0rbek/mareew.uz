...
import Brand from "#repo/brands.Model"
...
const GET_ONE = async (req, res) => { 
	try{
		...
		const brand = await Brand.GET_ONE({...req.params})
...
const POST = async (req, res) => { 
	try{
		...
		if(await Brand.checkBrand({ ...req.body })) throw new Error("Brand already exists")

		const brands = await Brand.POST({ ...req.body });
...
const PUT = async (req, res) => {
	try{
		...
		const brand = await Brand.PUT({...req.body});
...
const DELETE = async (req, res) => {
	try{
		...
		const brand = await Brand.DELETE({...req.body});
