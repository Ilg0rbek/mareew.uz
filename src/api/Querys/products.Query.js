const GET_ALL = `
	select 
		...
		p.product_safe_price,
		p.product_shipping_price
	from products p
...
const GET = `
	...
	join brands b on p.product_brand_id = b.brand_id 
	join categories c on p.product_category_id = c.category_id
	where product_deleted_at is null limit $1::int offset $2::int
`
...
const GET_COMMENTS = `
	...
	where comment_status = 'approved' and comment_deleted_at is null and comment_product_id = $1::int
`
...
const POST = `
	...
	($1::varchar, $2::int, $3::int, $4::int, $5::json, $6::json, $7::json, $8::varchar, $9::int, $10::int, $11::int, $12::int, $13::int) 
	returning *
`
...
const PUT = `
	update products set
		...
		product_safe_price = (
			case
				when $13::int is not null then $13::int
				else product_safe_price
			end
		),
		product_shipping_price = (
			case
				when $14::int is not null then $14::int
				else product_shipping_price
			end
		),
